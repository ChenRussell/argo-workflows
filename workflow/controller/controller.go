...
type WorkflowController struct {
	...
	archiveWfQueue        workqueue.RateLimitingInterface
...
func NewWorkflowController(ctx context.Context, restConfig *rest.Config, kubeclientset kubernetes.Interface, wfclientset wfclientset.Interface, namespace, managedNamespace, executorImage, executorImagePullPolicy, executorLogFormat, configMap string, executorPlugins bool) (*WorkflowController, error) {
	...
	wfc.archiveWfQueue = wfc.metrics.RateLimiterWithBusyWorkers(workqueue.DefaultControllerRateLimiter(), "archived_wf_queue")
...
func (wfc *WorkflowController) Run(ctx context.Context, wfWorkers, workflowTTLWorkers, podCleanupWorkers, cronWorkflowWorkers, archivedWorkflowWorkers int) {
	defer runtimeutil.HandleCrash(runtimeutil.PanicHandlers...)
	...
	log.WithField("workflowWorkers", wfWorkers).
		...
		WithField("archivedWorkflow", archivedWorkflowWorkers).
	...
	for i := 0; i < archivedWorkflowWorkers; i++ {
...
func (wfc *WorkflowController) processNextArchiveItem(ctx context.Context) bool {
	key, quit := wfc.archiveWfQueue.Get()
	if quit {
		return false
	}
	defer wfc.archiveWfQueue.Done(key)
