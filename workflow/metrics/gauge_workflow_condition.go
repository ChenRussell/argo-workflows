...
func (c *workflowConditionGauge) update(_ context.Context, o metric.Observer) error {
	...
	for condition, val := range conditions {
		c.gauge.ObserveInt(o, val, utilmetrics.InstAttribs{
			{Name: utilmetrics.LabelWorkflowType, Value: string(condition.Type)},
			{Name: utilmetrics.LabelWorkflowStatus, Value: string(condition.Status)},
		})
	}
	return nil
}
