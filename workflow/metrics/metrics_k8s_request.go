...
func (m metricsRoundTripper) RoundTrip(r *http.Request) (*http.Response, error) {
	...
	if x != nil && m.metrics != nil {
		...
		attribs := utilmetrics.InstAttribs{
			{Name: utilmetrics.LabelRequestKind, Value: kind},
			{Name: utilmetrics.LabelRequestVerb, Value: verb},
			{Name: utilmetrics.LabelRequestCode, Value: x.StatusCode},
		}
