...
func addOperationDurationHistogram(_ context.Context, m *Metrics) error {
	...
	return m.CreateInstrument(utilmetrics.Float64Histogram,
		...
		"s",
		utilmetrics.WithDefaultBuckets(prometheus.LinearBuckets(bucketWidth, bucketWidth, operationDurationMetricBucketCount)),
		utilmetrics.WithAsBuiltIn(),
	)
}

func (m *Metrics) OperationCompleted(ctx context.Context, durationSeconds float64) {
	m.Record(ctx, nameOperationDuration, durationSeconds, utilmetrics.InstAttribs{})
}
