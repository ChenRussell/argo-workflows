...
type realtimeTracker struct {
	inst *utilmetrics.Instrument
	key  string
}

func (cmv *customMetricValue) getLabels() utilmetrics.InstAttribs {
	labels := make(utilmetrics.InstAttribs, len(cmv.labels))
	for i := range cmv.labels {
		labels[i] = utilmetrics.InstAttrib{Name: cmv.labels[i].Key, Value: cmv.labels[i].Value}
	}
	return labels
}

func customUserdata(i *utilmetrics.Instrument, requireSuccess bool) map[string]*customMetricValue {
...
func getOrCreateValue(i *utilmetrics.Instrument, key string, labels []*wfv1.MetricLabel) *customMetricValue {
...
func (m *Metrics) GetCustomMetric(key string) *utilmetrics.Instrument {
	m.Mutex.RLock()
...
func (m *Metrics) matchExistingMetric(metricSpec *wfv1.Prometheus) (*utilmetrics.Instrument, error) {
	key := metricSpec.Name
...
func (m *Metrics) ensureBaseMetric(metricSpec *wfv1.Prometheus, ownerKey string) (*utilmetrics.Instrument, error) {
...
func (m *Metrics) createCustomMetric(metricSpec *wfv1.Prometheus) error {
	...
	case metricType == wfv1.MetricTypeHistogram:
		return m.CreateInstrument(utilmetrics.Float64Histogram, metricSpec.Name, metricSpec.Help, "{item}", utilmetrics.WithDefaultBuckets(metricSpec.Histogram.GetBuckets()))
	case metricType == wfv1.MetricTypeCounter:
		err := m.CreateInstrument(utilmetrics.Float64ObservableUpDownCounter, metricSpec.Name, metricSpec.Help, "{item}")
		if err != nil {
			return err
		}
		inst := m.AllInstruments[metricSpec.Name]
...
func (m *Metrics) createCustomGauge(metricSpec *wfv1.Prometheus) error {
	err := m.CreateInstrument(utilmetrics.Float64ObservableGauge, metricSpec.Name, metricSpec.Help, "{item}")
	if err != nil {
		return err
	}
	inst := m.AllInstruments[metricSpec.Name]
