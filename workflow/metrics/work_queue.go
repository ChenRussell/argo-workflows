...
type workersBusyRateLimiterWorkQueue struct {
	...
	workerType string
	busyGauge  *utilmetrics.Instrument
...
func addWorkQueueMetrics(_ context.Context, m *Metrics) error {
	err := m.CreateInstrument(utilmetrics.Int64UpDownCounter,
		nameWorkersBusy,
	...
	err = m.CreateInstrument(utilmetrics.Float64Histogram,
		nameWorkersQueueLatency,
		"Time objects spend waiting in the queue",
		"s",
		utilmetrics.WithDefaultBuckets([]float64{1.0, 5.0, 20.0, 60.0, 180.0}),
		utilmetrics.WithAsBuiltIn(),
	)
	if err != nil {
		return err
	...
	err = m.CreateInstrument(utilmetrics.Float64Histogram,
		...
		"s",
		utilmetrics.WithDefaultBuckets([]float64{0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 60.0, 180.0}),
		utilmetrics.WithAsBuiltIn(),
	)
	if err != nil {
		return err
	...
	err = m.CreateInstrument(utilmetrics.Float64ObservableGauge,
...
func (w *workersBusyRateLimiterWorkQueue) attributes() utilmetrics.InstAttribs {
	return utilmetrics.InstAttribs{{Name: utilmetrics.LabelWorkerType, Value: w.workerType}}
}
...
func (q *queueMetric) attributes() utilmetrics.InstAttribs {
	return utilmetrics.InstAttribs{{Name: utilmetrics.LabelQueueName, Value: q.name}}
}
...
func getQueueUserdata(i *utilmetrics.Instrument) *queueUserdata {
